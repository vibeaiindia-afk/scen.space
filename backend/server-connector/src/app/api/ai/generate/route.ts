...
export async function POST(req:NextRequest){
  try{
    ...
    const result=await generateWithRouting({feature,input:String(body?.input||''),system:typeof body?.system==='string'?body.system:undefined,maxOutputTokens:Number(body?.maxOutputTokens||0)||undefined,temperature:typeof body?.temperature==='number'?body.temperature:undefined});
